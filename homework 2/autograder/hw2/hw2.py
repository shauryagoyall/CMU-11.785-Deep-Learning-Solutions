...
class CNN(object):
    ...
    def __init__(self, input_width, num_input_channels, num_channels, kernel_sizes, strides,
        ...
        for i in range(len(num_channels)):
            # Create a Conv1D layer with given parameters
            conv = Conv1D(in_channel=in_channels, out_channel=num_channels[i],
                          kernel_size=kernel_sizes[i], stride=strides[i], weight_init_fn=conv_weight_init_fn, bias_init_fn=bias_init_fn)
        ...
        linear_input_size = num_channels[-1] * current_width
        self.linear_layer = Linear(linear_input_size, num_linear_neurons, weight_init_fn=linear_weight_init_fn, bias_init_fn=bias_init_fn)
    ...
    def forward(self, x):
        ...
        out = x
        for conv, act in zip(self.convolutional_layers, self.activations):
            out = conv(out)
            out = act(out)
    ...
    def backward(self, labels):
        ...
        intermediate_grads = [grad]
        conv_activation_grads = []

        for layer_idx in range(self.nlayers - 1, -1, -1):
            act_deriv = self.activations[layer_idx].backward()
            combined = intermediate_grads[self.nlayers - 1 - layer_idx] * act_deriv
            conv_activation_grads.append(combined)
            next_grad = self.convolutional_layers[layer_idx].backward(combined)
            intermediate_grads.append(next_grad)

        final_gradient = intermediate_grads[-1]
