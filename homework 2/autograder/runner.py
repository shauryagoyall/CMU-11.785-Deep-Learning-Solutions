...
# sys.path.append('C:\\Users\\shaur\\Desktop\\CMU DL\\homework 2\\mytorch')
# sys.path.append('C:\\Users\\shaur\\Desktop\\CMU DL\\homework 2\\hw2')
...
def conv2d_dilation_forward_backward_correctness(idx):
    ...
    norm = np.linalg.norm
    in_c, out_c = rint(5,15), rint(5,15)
    kernel, stride = rint(1,5), rint(1,5)
    batch, width = rint(1,4), rint(20,50)
    height = width
    ...
    ##########    Initialize the CNN layer and copy parameters to a PyTorch CNN layer   ##########
    ##############################################################################################
    def random_normal_weight_init_fn(out_channel, in_channel, kernel_size, kernel_size2):
        return np.random.normal(0, 1.0, (out_channel, in_channel, kernel_size, kernel_size2))

    if idx == 0:
        print("---Conv2D_dilation Forward (Tests)---")

    net = Conv2D_dilation(in_c, out_c, kernel, stride, padding, dilation, weight_init_fn=random_normal_weight_init_fn, bias_init_fn=np.ones)
    # except:
    #     info()
    #     print('Failed to pass parameters to your Conv2D_dilation function!')
    #     return scores_dict_1

    model = nn.Conv2d(in_channels=in_c, out_channels=out_c, kernel_size=kernel,
    ...
    #########################    Get the correct results from PyTorch   #########################
    #############################################################################################
    x = np.random.randn(batch, in_c, width, height)
    x1 = Variable(torch.tensor(x), requires_grad=True)
    y1 = model(x1)
    b, c, w, h = y1.shape
    delta = np.random.randn(b, c, w, h)
...
def conv2d_dilation_correctness():
    scores = []
    worker = min(mtp.cpu_count(),4)
    p = mtp.Pool(worker)

    for i in range(5):
        scores_dict_1 = conv2d_dilation_forward_backward_correctness(i)
    ...
    # scores = np.array(scores).min(0)
    a, b, c, d = np.array(scores).min(0)
    print('Section 4.3.1 - Forward')
    print('Conv2D_dilation Forward:', 'PASS' if a == 1 else 'FAIL')

    print('Section 4.3.2 - Backward')
...
def test_simple_scanning_mlp():
    data = np.loadtxt('data.asc').T.reshape(1, 24, -1)
    cnn = cnn_solution.CNN_SimpleScanningMLP()
    weights = np.load('mlp_weights_part_b.npy', allow_pickle = True)
    cnn.init_weights(weights)

    expected_result = np.load('res_b.npy', allow_pickle = True)
    result = cnn(data)
...
def test_distributed_scanning_mlp():
    ...
    weights = np.load('mlp_weights_part_c.npy', allow_pickle = True)
    cnn.init_weights(weights)

    expected_result = np.load('res_c.npy', allow_pickle = True)
    result = cnn(data)
...
tests = [
    ...
    },
    {
        ...
        'handler': conv2d_dilation_correctness,
        'value': 10,
    },
    {
