...
class CTC(object):
    ...
    def extend_target_with_blank(self, target):
        ...
        skip_connect.append(0)
        skip_connect = np.asarray(skip_connect)
    ...
    def get_backward_probs(self, logits, extended_symbols, skip_connect):
        ...
        for t in range(T - 2, -1, -1):
            ...
            for i in range(S - 2, -1, -1):
                beta[t, i] = beta[t + 1, i] * logits[t + 1, extended_symbols[i]] + beta[t + 1, i + 1] * logits[
                    t + 1, extended_symbols[i + 1]]
...
class CTCLoss(object):
    ...
    def forward(self, logits, target, input_lengths, target_lengths):
        ...
        for batch_itr in range(B):
            ...
            for t in range(T):
                for i in range(S):
                    total_loss[batch_itr] -= gamma[t, i] * np.log(logits[t, extended_symbols[i]])
    ...
    def backward(self):
        ...
        for batch_itr in range(B):
            ...
            for t in range(T):
                for i in range(S):
                    dY[t, batch_itr, extended_symbols[i]] -= gamma[t, i] / logits[t, extended_symbols[i]]
