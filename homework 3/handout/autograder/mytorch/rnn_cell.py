...
class RNNCell(object):
    ...
    def forward(self, x, h_prev_t):
        ...
        h_t = self.activation( x @ self.W_ih.T + self.b_ih + h_prev_t @ self.W_hh.T + self.b_hh )
    ...
    def backward(self, delta, h_t, h_prev_l, h_prev_t):
        ...
        dz = delta * self.activation.backward(h_t)   ### dY/dh * da/dz    (where a = f(z))

        self.dW_ih += (dz.T @ h_prev_l) / batch_size  # for generic implementation with stacked RNN, h_prev_l is used
        #### for rnn with only one later, h_prev_l is simply the input at that timestep
        self.dW_hh += (dz.T @ h_prev_t) / batch_size  # Average over batch

        self.db_ih += np.sum(dz, axis=0) / batch_size  # Sum across batch, then average
